...
// ---------------------------------------------------------------------------
check('tarih: tek yil MO', $call('dateRangeLabel', [-400, -400, 'tr']) === 'MO 400');
check('tarih: MO araligi tek kez donem yazar', $call('dateRangeLabel', [-400, -370, 'tr']) === 'MO 400-370');
check('tarih: MS araligi', $call('dateRangeLabel', [100, 150, 'tr']) === 'MS 100-150');
check('tarih: cag gecisi iki donem yazar', $call('dateRangeLabel', [-30, 14, 'tr']) === 'MO 30 - MS 14');
...
check('tarih: yalniz from dolu', $call('dateRangeLabel', [-250, null, 'tr']) === 'MO 250');
...
check('metal: silver -> gumus', $call('metalLabel', ['silver', 'tr']) === 'gumus');
...
check('ozet: bolge kunyede', strpos($t, 'Karya bolgesi') !== false);
check('ozet: darphane kunyede', strpos($t, 'Cnidus darphanesi') !== false);
check('ozet: tarih araligi kunyede', strpos($t, 'MO 100-28') !== false);
check('ozet: metal kunyede', strpos($t, 'bronz') !== false);
check('ozet: ON YUZ tasviri metne girer', strpos($t, 'On yuz: Head of Athena wearing a helmet') !== false);
check('ozet: ARKA YUZ tasviri metne girer', strpos($t, 'Arka yuz: Nike advancing holding a wreath') !== false);
check('ozet: url verilir', strpos($t, 'https://numistr.org/tr/anatolian-coins/caria-coins/6643-cnidus') !== false);
check('ozet: yuksek guvende uyari cikmaz', strpos($t, 'Benzerlik dusuk') === false);
...
check('grounding: bos on yuz icin satir YOK', strpos($t2, 'On yuz:') === false);
check('grounding: bos arka yuz icin satir YOK', strpos($t2, 'Arka yuz:') === false);
...
check('dusuk guven: ipucu uyarisi eklenir', strpos($r3['text'], 'Benzerlik dusuk') !== false);
...
check('coklu: diger eslesmeler basligi', strpos($t4, 'Diger yakin eslesmeler') !== false);
...
check('coklu: yardimci satirda bolge/tarih', strpos($t4, 'Iyonya') !== false && strpos($t4, 'MO 350-300') !== false);
...
check('eslesme yok: yardimci mesaj', strpos($r5['text'], 'eslestiremedim') !== false);
check('eslesme yok: baslik fallback', $r5['title'] === 'Sikke tanima');
